...
function boot(){
 ...
 const dom=new JSDOM(template,{runScripts:'outside-only',url:'http://localhost/admin/carteira',virtualConsole:vc});
 const w=dom.window;
 Object.assign(w,{D:data.dados,FICHA:data.fichas,INTER:data.inter,TAREFAS:data.tarefas,ALIASES:data.aliases,CANDIDATOS:data.candidatos,PROSPEC:data.prospec,REGIOES:data.regioes,ETAPAS:data.etapas,HOJE:data.hoje});
...
async function test(name,fn){let c=boot();try{await fn(c);assert.deepEqual(c.errors,[]);passed++;console.log('OK '+name);}finally{c.close();}}
(async()=>{
 await test('All screens render and agenda combines tasks with lead returns',c=>{
  assert.equal(c.api.agendaItens().length,3);
 ...
 await test('No-answer attempt needs next action and date',async c=>{
  ...
  await c.api.gravarContato(id);assert.equal(c.calls.length,0);
  fill(c,'fc-proximo','Tentar novamente');fill(c,'fc-prazo','2026-09-24');await c.api.gravarContato(id);
  assert.equal(c.calls[0].body.resultado,'sem_resposta');assert.equal(c.calls[0].body.proximo_em,'2026-09-24');
 });
